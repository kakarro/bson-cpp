...
const char *skipValue(const char *elem, const char *valueStart) {
  ...
  q(log() << "skipping ahead z:" << z << " type:" << (unsigned)type
          << endl;) if (z < 0x80) {
    p += z;
    ...
    if (z == 0xff) {
      ...
      int extra = (z & 0x7f);
      len += extra;
      q(log() << "will skip:" << len << " extra was:" << extra << endl;) p +=
          len;
    }
...
BsonElement BsonObj::getField(const StringData &name) const {
  ...
  while (1) {
    ...
    q(log() << "type " << (int)*elem << endl;) if (nul >= end) {
      q(log() << "end" << endl;) break;
    }
    if (*nul == 0) {
      // potential match
      q(log() << "memcmp check " << *p
              << endl;) if (memcmp(_name, p, name_sz) == 0) {
        q(log() << "match!" << endl;) BsonElement e =
            BsonElement(name_sz + 1, elem);
        q(log() << e.toString() << endl;) return e;
      }
    }
    // else, mismatch.  skip to next
    q(log() << "skip ahead:" << p << endl;) while (1) {
    ...
    if (*p == 0) {
      q(log() << "*p==EOO" << endl;) break;
    }
  }
  q(log() << "returning BSONElement() (mismatch)"
          << endl;) return BsonElement();
}
...
// This is to ensure that BsonObjBuilder doesn't try to use numStrs before the
// strings have been constructed I've tested just making numStrs a char[][], but
// the overhead of constructing the strings each time was too high numStrsReady
// will be 0 until after numStrs is initialized because it is a static variable
...
void BsonElement::toString(StringBuilder &s, bool includeFieldName, bool full,
                           int depth) const {
  ...
  if (includeFieldName && type() != EOO) s << fieldName() << ": ";
  switch (type()) {
    case EOO:
      //  ??
      s << "EOO";
    ...
    case DBRef:
      s << "DBRef('" << valuestr() << "',";
      {
        QBson::OID *x = (QBson::OID *)(valuestr() + valuestrsize());
        s << x->toString() << ')';
      }
      break;
    case jstOID:
...
void BsonObj::toString(StringBuilder &s, bool isArray, bool full,
                       int depth) const {
  if (isEmpty()) {
    s << (isArray ? "[]" : "{}");
    return;
...
int BsonElement::size(int maxLen) const {
  ...
  switch (type()) {
    ...
    case RegEx: {
      ...
      // massert( 10318 ,  "Invalid regex string", len1 != -1 ); // ERH -
      // 4/28/10 - don't think this does anything
      p = p + len1 + 1;
      size_t len2;
      if (maxLen == -1)
        len2 = strlen(p);
      ...
      // massert( 10319 ,  "Invalid regex options string", len2 != -1 ); // ERH
      // - 4/28/10 - don't think this does anything
      x = (int)(len1 + 1 + len2 + 1);
    } break;
    default: {
...
BsonObjBuilder &BsonObjBuilder::appendElementsUnique(BsonObj x) {
  std::set<std::string> have;
  {
    ...
    while (i.more()) have.insert(i.next().fieldName());
  }

  BsonObjIterator it(x);
  while (it.more()) {
    BsonElement e = it.next();
    if (have.count(e.fieldName())) continue;
    append(e);
  }
  return *this;
}
...
inline int compareElementValues(const BsonElement &l, const BsonElement &r) {
  ...
  switch (l.type()) {
    ...
    case Timestamp:
      // unsigned compare for timestamps - note they are not really dates but
      // (ordinal + time_t)
    ...
    case BinData: {
      int lsz = l.objsize();  // our bin data size in bytes, not including the
                              // subtype byte
      int rsz = r.objsize();
      if (lsz - rsz != 0) return lsz - rsz;
      return memcmp(l.value() + 4, r.value() + 4,
                    lsz + 1 /*+1 for subtype byte*/);
    }
    case RegEx: {
