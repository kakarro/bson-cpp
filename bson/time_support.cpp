...
StringData getNextToken(const StringData& currentString,
                        const StringData& terminalChars, size_t startIndex,
                        size_t* endIndex) {
  ...
  // substr just returns the rest of the string if the length passed in is
  // greater than the number of characters remaining, and since
  // std::string::npos is the length of the largest possible string we know
  // (std::string::npos - startIndex) is at least as long as the rest of the
  // string.  That means this handles both the case where we hit a terminating
  // character and we want a substring, and the case where didn't and just want
  ...
  *tzAdjSecs = 0;
  ...
  if (!tzStr.empty()) {
    if (tzStr[0] == 'Z') {
      ...
      // parseNumberFromStringWithBase correctly handles the sign bit, so leave
      // that in.
      StringData tzHoursStr = tzStr.substr(0, 3);
      int tzAdjHours = 0;
      Status status =
          parseNumberFromStringWithBase(tzHoursStr, 10, &tzAdjHours);
      if (!status.isOK()) {
        return status;
      }
      ...
      // Use the sign that parseNumberFromStringWithBase found to determine if
      // we need to flip the sign of our minutes component.  Also, we need to
      // flip the sign of our final result, because the offset passed in by the
      // user represents how far off the time they are giving us is from UTC,
      // which means that we have to go the opposite way to compensate and get
      // the UTC time
      *tzAdjSecs = (-1) * ((tzAdjHours < 0 ? -1 : 1) * (tzAdjMinutes * 60) +
                           (tzAdjHours * 60 * 60));

      // Disallow adjustiment of 24 hours or more in either direction (should be
      // checked above as the separate components of minutes and hours)
...
Status parseMillisFromToken(const StringData& millisStr, int* resultMillis) {
  *resultMillis = 0;
  ...
  if (!millisStr.empty()) {
    ...
    // Treat the digits differently depending on how many there are.  1 digit =
    // hundreds of milliseconds, 2 digits = tens of milliseconds, 3 digits =
    // milliseconds.
...
Status parseTmFromTokens(const StringData& yearStr, const StringData& monthStr,
                         const StringData& dayStr, const StringData& hourStr,
                         const StringData& minStr, const StringData& secStr,
                         std::tm* resultTm) {
  ...
  Status status =
      parseNumberFromStringWithBase(yearStr, 10, &resultTm->tm_year);
  if (!status.isOK()) {
    return status;
  }
...
Status parseTm(const StringData& dateString, std::tm* resultTm,
               int* resultMillis, int* tzAdjSecs) {
  ...
  StringData yearStr, monthStr, dayStr, hourStr, minStr, secStr, millisStr,
      tzStr;
  ...
  // Only look for seconds if the character we matched for the end of the
  // minutes token is a colon
  if (minEnd != std::string::npos && dateString[minEnd] == ':') {
    // Make sure the string doesn't end with ":"
    if (minEnd == dateString.size() - 1) {
      StringBuilder sb;
      sb << "Invalid date:  " << dateString << ".  Ends with \""
         << dateString[minEnd] << "\" character";
      return Status(BadValue, sb.str());
    }

    secStr = getNextToken(dateString, ".+-Z", minEnd + 1, &secEnd);

    // Make sure we actually got something for seconds, since here we know they
    // are expected
    if (secStr.empty()) {
      StringBuilder sb;
  ...
  // Only look for milliseconds if the character we matched for the end of the
  // seconds token is a period
  if (secEnd != std::string::npos && dateString[secEnd] == '.') {
    // Make sure the string doesn't end with "."
    if (secEnd == dateString.size() - 1) {
      StringBuilder sb;
      sb << "Invalid date:  " << dateString << ".  Ends with \""
         << dateString[secEnd] << "\" character";
      return Status(BadValue, sb.str());
    }

    millisStr = getNextToken(dateString, "+-Z", secEnd + 1, &millisEnd);

    // Make sure we actually got something for millis, since here we know they
    // are expected
    if (millisStr.empty()) {
      StringBuilder sb;
      sb << "Missing seconds in date: " << dateString;
  ...
  // Now look for the time zone specifier depending on which prefix of the time
  // we provided
  ...
  Status status = parseTmFromTokens(yearStr, monthStr, dayStr, hourStr, minStr,
                                    secStr, resultTm);
  if (!status.isOK()) {
    return status;
  }
...
StatusWith<Date_t> dateFromISOString(const StringData& dateString) {
  ...
  if (SystemTimeToFileTime(&dateStruct, &fileTime) == 0) {
    StringBuilder sb;
    sb << "Error converting Windows system time to file time for date:  "
       << dateString << ".  Error code:  " << GetLastError();
  ...
  resultMillis =
      (1000 * static_cast<unsigned long long>(timegm(&dateStruct))) + millis;
#endif
