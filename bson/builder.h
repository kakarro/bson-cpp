 ...
 public:
  ...
  void* Realloc(void* p, size_t sz) {
    if (p == buf) {
      if (sz <= SZ) return buf;
      void* d = malloc(sz);
      if (d == 0) msgasserted(15912, "out of memory StackAllocator::Realloc");
      memcpy(d, p, SZ);
      return d;
    }
    return realloc(p, sz);
...
class _BufBuilder {
 ...
 public:
  _BufBuilder(int initsize = 512) : size(initsize) {
    if (size > 0) {
      ...
      if (data == 0) msgasserted(10000, "out of memory BufBuilder");
    } else {
      data = 0;
    }
    l = 0;
  ...
  void reset(int maxSize) {
    ...
    if (maxSize && size > maxSize) {
      ...
      if (data == 0) msgasserted(15913, "out of memory BufBuilder::reset");
      size = maxSize;
    }
  }

  /** leave room for some stuff later
      @return point to region that was skipped.  pointer may change later (on
     realloc), so for immediate use only
  */
  ...
  void appendNum(short j) { *((short*)grow(sizeof(short))) = endian_short(j); }
  void appendNum(int j) { *((int*)grow(sizeof(int))) = endian_int(j); }
  void appendNum(unsigned j) {
    *((unsigned*)grow(sizeof(unsigned))) = endian(j);
  }
  void appendNum(bool j) { *((char*)grow(sizeof(char))) = j ? 1 : 0; }
 ...
 private:
  ...
  void NOINLINE_DECL grow_reallocate(int newLen) {
    int a = 64;
    while (a < newLen) a = a * 2;
    if (a > BufferMaxSize) {
      std::stringstream ss;
      ss << "BufBuilder attempted to grow() to " << a
...
/** std::stringstream deals with locale so this is a lot faster than
 * std::stringstream for UTF8 */
template <typename Allocator>
class StringBuilderImpl {
 public:
