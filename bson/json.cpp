...
#define MONGO_JSON_DEBUG(message)                                \
  log() << "JSON DEBUG @ " << __FILE__ << ":" << __LINE__ << " " \
        << __FUNCTION__ << ": " << message << endl;
#else
#define MONGO_JSON_DEBUG(message)
#endif
...
Status JParse::err() { return parseError("can't parse"); }
...
Status JParse::object(const StringData& fieldName, BsonObjBuilder& builder,
                      bool subObject) {
  MONGO_JSON_DEBUG("fieldName: " << fieldName);
  if (!readToken(LBRACE)) {
...
Status JParse::objectIdObject(const StringData& fieldName,
...
Status JParse::binaryObject(const StringData& fieldName,
                            BsonObjBuilder& builder) {
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }
  ...
  if (!readField("$type")) {
    return parseError(
        "Expected second field name: \"$type\", in \"$binary\" object");
  }
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }
  ...
  if ((binDataType.size() != 2) || !isHexString(binDataType)) {
    return parseError(
        "Argument of $type in $bindata object must be a hex string "
        "representation of a single byte");
  }
  builder.appendBinData(fieldName, binData.length(),
                        BinDataType(fromHex(binDataType)), binData.data());
  return Status::OK();
}

Status JParse::dateObject(const StringData& fieldName,
                          BsonObjBuilder& builder) {
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }
  ...
  if (peekToken(DOUBLEQUOTE)) {
    ...
    if (fieldName != "$numberLong") {
      return parseError(
          "Expected field name: $numberLong for $date value object");
    }
    if (!readToken(COLON)) {
      return parseError("Expected ':'");
    }

    // The number must be a quoted string, since large long numbers could
    // overflow a double and thus may not be valid JSON
    ...
    // requires that we know ahead of time where the number ends, which is not
    // currently the case.
    ...
    if (errno == ERANGE) {
      /* Need to handle this because jsonString outputs the value of Date_t as
       * unsigned. See SERVER-8330 and SERVER-8573 */
      errno = 0;
      // SERVER-11920: We should use parseNumberFromString here, but that
      // function requires that we know ahead of time where the number ends,
...
Status JParse::timestampObject(const StringData& fieldName,
                               BsonObjBuilder& builder) {
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }
  ...
  assert(false);
  // builder.appendTimestamp(fieldName, (static_cast<uint64_t>(seconds))*1000,
  // count);
  return Status::OK();
}

Status JParse::regexObject(const StringData& fieldName,
                           BsonObjBuilder& builder) {
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }
  ...
  if (readToken(COMMA)) {
    if (!readField("$options")) {
      return parseError(
          "Expected field name: \"$options\" in \"$regex\" object");
    }
    if (!readToken(COLON)) {
      return parseError("Expected ':'");
    }
...
Status JParse::dbRefObject(const StringData& fieldName,
                           BsonObjBuilder& builder) {
  ...
  if (readToken(COMMA)) {
    if (!readField("$db")) {
      return parseError(
          "DBRef: Expected field name: \"$db\" in \"$ref\" object");
    }
    if (!readToken(COLON)) {
      return parseError("DBRef: Expected ':'");
    }
...
Status JParse::undefinedObject(const StringData& fieldName,
                               BsonObjBuilder& builder) {
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }
...
Status JParse::numberLongObject(const StringData& fieldName,
                                BsonObjBuilder& builder) {
  if (!readToken(COLON)) {
    return parseError("Expected ':'");
  }

  // The number must be a quoted string, since large long numbers could overflow
  // a double and thus may not be valid JSON
  std::string numberLongString;
  numberLongString.reserve(NUMBERLONG_RESERVE_SIZE);
...
Status JParse::constructor(const StringData& fieldName,
                           BsonObjBuilder& builder) {
  if (readToken("Date")) {
...
Status JParse::numberLong(const StringData& fieldName,
                          BsonObjBuilder& builder) {
  if (!readToken(LPAREN)) {
    return parseError("Expected '('");
  }
...
Status JParse::number(const StringData& fieldName, BsonObjBuilder& builder) {
  ...
  // note: We should use parseNumberFromString here, but that function requires
  // that we know ahead of time where the number ends, which is not currently
  // the case.
...
Status JParse::field(std::string* result) {
  ...
  if (peekToken(DOUBLEQUOTE) || peekToken(SINGLEQUOTE)) {
    ...
    // Unquoted key
    // 'isspace()' takes an 'int' (signed), so (default signed) 'char's get
    // sign-extended and therefore 'corrupted' unless we force them to be
    // unsigned ... 0x80 becomes 0xffffff80 as seen by isspace when
    // sign-extended ... we want it to be 0x00000080
...
std::string JParse::encodeUTF8(unsigned char first,
...
string JParse::get(const char* chars_wanted) {
  ...
  while (1) {
    char ch = peek();
    if (strchr(chars_wanted, ch) == 0) break;
    getc();
    b << ch;
  }
  return b.str();
...
char JParse::peek() { return _in.peek(); }
...
bool JParse::readTokenImpl(const char* token) {
  ...
  // 'isspace()' takes an 'int' (signed), so (default signed) 'char's get
  // sign-extended and therefore 'corrupted' unless we force them to be unsigned
  // ... 0x80 becomes 0xffffff80 as seen by isspace when sign-extended ... we
  // want it to be 0x00000080
