...
/**
 * Returns the value of the digit "c", with the same conversion behavior as
 * strtol.
 *
...
/**
 * Assuming "stringValue" represents a parseable number, extracts the sign and
 * returns a substring with any sign characters stripped away.  "*isNegative" is
 * set to true if the number is negative, and false otherwise.
 */
static inline StringData _extractSign(const StringData& stringValue,
                                      bool* isNegative) {
  ...
  if (foundSignMarker) return stringValue.substr(1);
  return stringValue;
}

/**
 * Assuming "stringValue" represents a parseable number, determines what base to
 * use given "inputBase".  Stores the correct base into "*outputBase".  Follows
 * strtol rules.  If "inputBase" is not 0, *outputBase is set to "inputBase".
 * Otherwise, if "stringValue" starts with "0x" or "0X", sets outputBase to 16,
 * or if it starts with 0, sets outputBase to 8.
 *
 * Returns stringValue, unless it sets *outputBase to 16, in which case it will
 * strip off the "0x" or "0X" prefix, if present.
 */
...
Status parseNumberFromStringWithBase(const StringData& stringValue, int base,
                                     NumberType* result) {
  ...
  StringData str =
      _extractBase(_extractSign(stringValue, &isNegative), base, &base);

  if (str.empty()) return Status(FailedToParse, "No digits");
  ...
  if (isNegative) {
    if (limits::is_signed) {
      for (size_t i = 0; i < str.size(); ++i) {
        ...
        if (int(digitValue) >= base) return Status(FailedToParse, "Bad digit");

// MSVC: warning C4146: unary minus operator applied to unsigned type, result
// still unsigned This code is statically known to be dead when NumberType is
// unsigned, so the warning is not real
#pragma warning(push)
...
// Definition of the various supported implementations of
// parseNumberFromStringWithBase.

#define DEFINE_PARSE_NUMBER_FROM_STRING_WITH_BASE(NUMBER_TYPE) \
  template Status parseNumberFromStringWithBase<NUMBER_TYPE>(  \
      const StringData&, int, NUMBER_TYPE*);
...
/**
 * Converts ascii c-locale uppercase characters to lower case, leaves other char
 * values unchanged.
 */
char toLowerAscii(char c) {
  if (isascii(c) && isupper(c)) return _tolower(c);
  return c;
}
...
Status parseNumberFromStringWithBase<double>(const StringData& stringValue,
                                             int base, double* result) {
  if (base != 0) {
    return Status(
        BadValue,
        "Must pass 0 as base to parseNumberFromStringWithBase<double>.");
  }
  if (stringValue.empty()) return Status(FailedToParse, "Empty string");
...
#ifdef _WIN32
    // The Windows libc implementation of strtod cannot parse +/-infinity or
    // nan, so handle that here.
  ...
  if (actualErrno == ERANGE) return Status(FailedToParse, "Out of range");
  *result = d;
  return Status::OK();
}
