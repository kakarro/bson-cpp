...
inline unsigned endian(unsigned x) { return big ? _byteswap_ulong(x) : x; }
inline short endian_short(short x) { return big ? _byteswap_ushort(x) : x; }
...
inline int endian_int(int x) { return endian(x); }
