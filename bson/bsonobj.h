...
/**
   ...
   The raw bson buffer referenced is treated as immutable.  There is no
   transformation -- we aren't creating a DOM or anything -- just scanning the
   BSON upon request and such.
...
class BsonObj {
 private:
  ...
  void init(const char* data) {
    _objdata = data;
    if (!isValid()) _assertInvalid();
  }

 public:
  /** Construct a BsonObj from data in the proper format.
   */
  explicit BsonObj(const char* msgdata) { init(msgdata); }
  ...
  /** Readable representation of a BSON object in an extended JSON-style
     notation. This is an abbreviated representation which might be used for
     logging.
  */
  ...
  */
  ...
  /** @return the specified element.  element.eoo() will be true if not found.
      @param name field to find. supports dot (".") notation to reach into
     embedded objects. for example "x.y" means "in the nested object in field x,
     retrieve field y"
  */
  BsonElement getFieldDotted(const StringData& name) const;

  /** Like getFieldDotted(), but expands arrays and returns all matching
   * objects. Turning off expandLastArray allows you to retrieve nested array
   * objects instead of their contents.
   */
  // void getFieldsDotted(const StringData& name, BSONElementSet &ret, bool
  // expandLastArray = true ) const; void getFieldsDotted(const StringData&
  // name, BSONElementMSet &ret, bool expandLastArray = true ) const;

  /** Like getFieldDotted(), but returns first array encountered while
     traversing the dotted fields of name.  The name variable is updated to
     represent field names with respect to the returned element. */
  ...
  */
  ...
  /** Get several fields at once. This is faster than separate getField() calls
     as the size of elements iterated can then be calculated only once each.
     ...
     corresponding position in this array. if not found the array element is
     unchanged.
   */
  void getFields(unsigned n, const char** fieldNames,
  ...
  */
  // todo fix retval
  BsonObj extractFields(const BsonObj& pattern,
                        bool fillWithNull = false) const;
  ...
  int objsize() const { return readInt(objdata()); }
  ...
  */
  ...
  /**
   * @param useDotted whether to treat sort key fields as possibly dotted and
   * expand into them
   */
  ...
  */
  ...
  /** faster than firstElement().fieldName() - for the first element we can
     easily find the fieldname without computing the element size.
  */
  ...
  /** Get the _id field from the object.  For good performance drivers should
      assure that _id is the first element of the object; however, correct
  ...
  */
  ...
  int hash() const {
    ...
    for (int i = 0; i < objsize(); i++) x = x * 131 + p[i];
  ...
  /** add all values of the object to the specified vector.  If type mismatches,
     exception. this is most useful when the BsonObj is an array, but can be
     used with non-arrays too in theory.
  ...
  */
  ...
  /** add all values of the object to the specified list.  If type mismatches,
   * exception. */
  template <class T>
