...
class BsonObjBuilder {
 ...
 public:
  ...
  BsonObjBuilder& append(const BsonElement& e) {
    verify(!e.eoo());  // do not append eoo, that would corrupt us. the builder
                       // auto appends when done() is called.
  ...
  /** add a subobject as a member */
  BsonObjBuilder& appendObject(const StringData& fieldName, const char* objdata,
                               int size = 0) {
    verify(objdata != 0);
    if (size == 0) {
      size = *((int*)objdata);
  ...
  /** add a subobject as a member with type Array.  Thus arr object should have
  "0", "1", ... style fields in it.
  */
  ...
  */
  BsonObjBuilder& appendDate(const StringData& fieldName, Date_t dt) {
    ...
    if (dt > 0 && dt <= 0xffffffff) {
      ...
      if (n++ == 0)
        log()
            << "DEV WARNING appendDate() called with a tiny (but nonzero) date"
            << std::endl;
    }
#endif
    _b.appendNum((char)Date);
  ...
  */
  ...
  /**
   * Alternative way to store an OpTime in BSON. Pass the OpTime as a Date, as
   * follows:
   *
  ...
  */
  /** Append a binary data element
      ...
      @param subtype subtype information for the data. @see enum BinDataType in
     bsontypes.h. Use BinDataGeneral if you don't care about the type.
  ...
  */
   ...
   */
  BsonObj obj() { return BsonObj(_done()); }
  ...
  */
  BsonObj done() { return BsonObj(_done()); }

  /** Peek at what is in the builder, but leave the builder ready for more
  appends. The returned object is only valid until the next modification or
  destruction of the builder. Intended use case: append a field if not already
  there.
  */
  BsonObj asTempObj() {
    BsonObj temp(_done());
  ...
  /** Make it look as if "done" has been called, so that our destructor is a
   * no-op. Do this if you know that you don't care about the contents of the
   * builder you are destroying.
   *
   *  Note that it is invalid to call any method other than the destructor after
   * invoking this method.
   */
  void abandon() { _doneCalled = true; }

  void appendKeys(const BsonObj& keyPattern, const BsonObj& values);

  static std::string numStr(int i) {
    if (i >= 0 && i < 100 && numStrsReady) return numStrs[i];
    StringBuilder o;
    o << i;
    return o.str();
  }
  ...
  /** @return true if we are using our own bufbuilder, and not an alternate that
   * was given to us in our constructor */
 ...
 private:
  ...
  static bool
      numStrsReady;  // for static init safety. see comments in db/jsobj.cpp
};

template <class L>
inline BsonObjBuilder& _appendIt(BsonObjBuilder& _this,
                                 const StringData& fieldName, const L& vals) {
...
inline BsonObjBuilder& BsonObjBuilder::append(const StringData& fieldName,
                                              const std::map<K, T>& vals) {
  BsonObjBuilder bob;
  for (typename std::map<K, T>::const_iterator i = vals.begin();
       i != vals.end(); ++i) {
