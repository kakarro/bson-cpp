 ...
 *
 *  + Because string data can be used to pass a substring around, one should
 * never assume a rawData() terminates with a null.
 */
class StringData {
 public:
  ...
  /**
   * Constructs a StringData, for the case where the length of string is not
   * known. 'c' must be a pointer to a null-terminated string.
   */
  StringData(const char* c)
      : _data(c), _size((c == NULL) ? 0 : std::string::npos) {}

  /**
   * Constructs a StringData explicitly, for the case where the length of the
   * string is already known. 'c' must be a pointer to a null-terminated string,
   * and len must be the length that strlen(c) would return, a.k.a the index of
   * the terminator in c.
   */
  ...
  /**
   * Constructs a StringData explicitly, for the case of a literal whose size is
  ...
  /**
   * Returns true if 'prefix' is a substring of this instance, anchored at
   * position 0.
   */
  bool startsWith(const StringData& prefix) const;

  /**
   * Returns true if 'suffix' is a substring of this instance, anchored at the
   * end.
   */
  ...
  /**
   * Get the pointer to the first byte of StringData.  This is not guaranteed to
   * be null-terminated, so if using this without checking size(), you are
   ...
   * Warning: The hash function is subject to change. Do not use in cases where
   * hashes need to be consistent across versions.
   */
  struct Hasher {
 ...
 private:
  const char*
      _data;  // is not guaranted to be null terminated (see "notes" above)
...
/* inline std::stringstream& operator<<(std::stringstream& stream, const
   StringData& value) { stream << value.toString(); return stream;
      }*/
/*
