...
/** BsonElement represents an "element" in a BsonObj.  So for the object { a :
   3, b : "abc" }, 'a : 3' is the first element (key+value).

    The BsonElement object points into the BsonObj's data.  Thus the BsonObj
...
class BsonElement {
 ...
 public:
  ...
  /** These functions, which start with a capital letter, throw a
     MsgAssertionException if the element is not of the required type. Example:

      std::string foo = obj["foo"].String(); // std::exception if not a
     std::string type or DNE
  */
  std::string String() const { return chk(QBson::String).str(); }
  Date_t Date() const { return chk(QBson::Date).date(); }
  ...
  bool Bool() const { return chk(QBson::Bool).boolean(); }
  std::vector<BsonElement> Array()
      const;  // see implementation for detailed comments
  QBson::OID OID() const { return chk(jstOID).__oid(); }
  void Null() const {
    chk(isNull());
  ...
  /** @return the embedded object associated with this field.
      Note the returned object is a reference to within the parent bson object.
     If that object is out of scope, this pointer will no longer be valid. Call
     getOwned() on the returned BsonObj if you need your own copy. throws
     UserException if the element is not of type object.
  */
  BsonObj object() const;
  // BsonObj Obj() const;

  /** populate v with the value of the element.  If type does not match, throw
     exception. useful in templates -- see also BsonObj::Vals().
  */
  void Val(Date_t& v) const { v = Date(); }
  void Val(long long& v) const { v = Long(); }
  void Val(bool& v) const { v = Bool(); }
  void Val(BsonObj& v) const;
  void Val(QBson::OID& v) const { v = OID(); }
  ...
  */
  ...
  std::string jsonString(JsonStringFormat format, bool includeFieldNames = true,
                         int pretty = 0) const;
  ...
  */
  ...
  /** Indicates if it is the end-of-object element, which is present at the end
     of every BSON object.
  */
  bool eoo() const { return type() == EOO; }

  /** Size of the element.
      @param maxLen If maxLen is specified, don't scan more than maxLen bytes to
     calculate size.
  */
  ...
  */
  ...
  int fieldNameSize() const {
    if (fieldNameSize_ == -1) fieldNameSize_ = (int)strlen(fieldName()) + 1;
  ...
  bool isBoolean() const { return type() == QBson::Bool; }
  ...
  */
  ...
  /** Convert the value to boolean, regardless of its type, in a javascript-like
     fashion (i.e., treats zero and null and eoo as false).
  */
  ...
  long long _numberLong() const {
    return endian_ll(*reinterpret_cast<const long long*>(value()));
  }
   ...
   * number. Behavior is not defined for double values that are NaNs, or too
   * large/small to be represented by long longs */
  ...
  /** Retrieve the numeric value of the element.  If not of a numeric type,
     returns 0. Note: casts to double, data loss may occur with large (>52 bit)
     NumberLong values.
  */
  ...
  */
  ...
  const QBson::OID& __oid() const {
    return *reinterpret_cast<const QBson::OID*>(value());
  }
  ...
  */
  int valuestrsize() const { return readInt(value()); }

  // for objects the size *includes* the size of the size field
  size_t objsize() const { return static_cast<const size_t>(readInt(value())); }

  /** Get a string's value.  Also gives you start of the real data for an
     embedded object. You must assure data is of an appropriate type first --
     see also valuestrsafe().
  */
  const char* valuestr() const { return value() + 4; }

  /** Get the string value of the element.  If not a string returns "". */
  const char* valuestrsafe() const {
    return type() == QBson::String ? valuestr() : "";
  }
  /** Get the string value of the element.  If not a string returns "". */
  std::string str() const {
    return type() == QBson::String ? std::string(valuestr(), valuestrsize() - 1)
                                   : std::string();
  }
  ...
  /** Get raw binary data.  Element must be of type BinData. Doesn't handle type
   * 2 specially */
  ...
  */
  ...
  unsigned int timestampInc() const { return ((unsigned int*)(value()))[0]; }
  ...
  };  // For disambiguation with ctor taking 'maxLen' above.

  /** Construct a BsonElement where you already know the length of the name. The
   * value passed here includes the null terminator. The data pointed to by 'd'
   * must not represent an EOO. You may pass -1 to indicate that you don't
  ...
  const BsonElement& chk(int t) const {
    if (t != type()) {
      ...
      if (eoo())
        ...
        ss << "wrong type for field (" << fieldName() << ") " << type()
           << " != " << t;
